...
dataset = load_dataset("csv", data_files=f"./data/CLEF_1C_{args.language}.tsv", delimiter="\t")
...
with open(f"./data/Clef_1C_DPP_{args.language}.json") as json_file:
...
def construct_few_shot_string(data, example_set, few_shot_n):
    few_shot_string = ""

    for label in data[example_set]:
        for n in range(few_shot_n):
...
for few_shot_set in few_shot_examples:
    ...
    for n in range(1, 5 + 1):
        print("-" * 10, f" Evaluating {n*2}-shot ", "-" * 10)
        results[few_shot_set][f"{n*2}_shot"] = {}

        few_shots_string = construct_few_shot_string(few_shot_examples, few_shot_set, n)
        ...
        results[few_shot_set][f"{n*2}_shot"] = evals
