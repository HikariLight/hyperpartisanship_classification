...
main_run = wandb.init(
    ...
    entity="michelej-m",
    name=f"[Specific] {args.model_name.split('/')[1]}_zero_shot",
)
...
prompts = {
    "english": {
        ...
        "user": "### Instruction: \n Detect if a tweet is harmful to society or not. The possible choices are: ’0’ if the article is neutral, ’1’ if the tweet is harmful.  Harmful tweet is a post on the social media platform Twitter that can cause distress, harm, or damage to individuals, groups, or society. This type of tweet may include content that is abusive, offensive, defamatory, threatening, or incites violence. Additionally, it may spread false information, contribute to cyberbullying, or perpetuate hate speech and discrimination. Harmful tweets can  negatively impact mental health, reputations, and community safety. The output of the label is only one integer like this example: ’integer’. \n###Input: {} \n###Response:",
    },
    "bulgarian": {
        ...
        "user": """### Инструкция:\n Открийте дали един туит е вреден за обществото или не. Възможните избори са: „0“, ако статията е неутрална, „1“, ако туитът е вреден. Вреден туит е публикация в социалната медийна платформа Twitter, която може да причини страдание, вреда или увреждане на отделни лица, групи или общество. Този тип туит може да включва съдържание, което е злоупотребяващо, обидно, клеветническо, заплашително или подбуждащо към насилие. Освен това може да разпространява невярна информация, да допринесе за кибертормоз или да увековечи речта на омразата и дискриминацията. Вредните туитове могат да повлияят отрицателно на психичното здраве, репутацията и безопасността на общността. Резултатът от етикета е само едно цяло число като този пример: „цяло число“\n ###Вход: {}\n ###Отговор:,\n""",
    },
    "arabic": {
        "system": "لقد تلقيت تعليمات تصف مهمة وتم دمجها مع مدخلات توفر سياقًا أكثر. استجب وفقًا للتوجيهات الواردة في التعليمات.",
        "user": """### التعليمات:\n اكتشف ما إذا كانت التغريدة ضارة بالمجتمع أم لا. الخيارات الممكنة هي: "0" إذا كانت المقالة محايدة، و"1" إذا كانت التغريدة ضارة. التغريدة الضارة هي منشور على منصة التواصل الاجتماعي تويتر يمكن أن يسبب ضائقة أو ضررًا أو إلحاق الضرر بالأفراد أو المجموعات أو المجتمع. قد يتضمن هذا النوع من التغريدات محتوى مسيءًا أو مهينًا أو تشهيريًا أو مهددًا أو يحرض على العنف. بالإضافة إلى ذلك، قد ينشر معلومات كاذبة، أو يساهم في التنمر الإلكتروني، أو يديم خطاب الكراهية والتمييز. يمكن أن تؤثر التغريدات الضارة سلبًا على الصحة العقلية والسمعة وسلامة المجتمع. ناتج العلامة هو عدد صحيح واحد فقط مثل هذا المثال: 'integer'.\n ###الإدخال: {}\n ###الاستجابة:""",
    },
}
