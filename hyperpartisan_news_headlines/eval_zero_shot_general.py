...
prompt = """
### Instruction: 
Determine if the article is hyperpartisan or not. The possible choices are: ’0’ if the article is neutral, ’1’ if the article is hyperpartisan.. 
...
for idx, element in enumerate(dataset["test"]):
    ...
    if parse_label(pred) is None:
        ...
        while True:
            pred = generate(
                model, tokenizer, prompt, element["text"], temperature=0.7, verbose=True
            )
